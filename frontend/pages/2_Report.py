...
st.set_page_config(page_title="Accuracy Report — FactCheck Engine", layout="wide", page_icon="📊")
...
.fc-card{background:#0d1424;border:1px solid #1e2d4a;border-radius:12px;padding:1.25rem;margin-bottom:1rem}
.claim-true{border-left:3px solid #10b981!important}
.claim-false{border-left:3px solid #ef4444!important}
.claim-partial{border-left:3px solid #f59e0b!important}
.claim-unverifiable{border-left:3px solid #475569!important}
...
if not result:
    st.markdown("""
    <div style="text-align:center;padding:4rem 0;font-family:IBM Plex Mono,monospace;color:#475569;">
        <div style="font-size:3rem;margin-bottom:1rem;">📊</div>
        <div style="font-size:1.2rem;font-weight:700;color:#64748b;">No Report Yet</div>
        <div style="font-size:0.85rem;margin-top:0.5rem;">Run a fact-check from the Home page first.</div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
VERDICT_META = {
    ...
    "PARTIALLY TRUE":{"color": "#f59e0b", "cls": "claim-partial",      "emoji": "⚠️", "bg": "rgba(245,158,11,0.1)"},
    "UNVERIFIABLE":  {"color": "#94a3b8", "cls": "claim-unverifiable", "emoji": "❔", "bg": "rgba(148,163,184,0.05)"},
    "OUTDATED":      {"color": "#a78bfa", "cls": "claim-unverifiable", "emoji": "🕰️", "bg": "rgba(167,139,250,0.08)"},
}
...
def verdict_badge_html(verdict: str) -> str:
    ...
    return (
        ...
        f'border-radius:5px;padding:2px 10px;font-family:IBM Plex Mono,monospace;'
        f'font-size:0.72rem;font-weight:600;">{m["emoji"]} {verdict}</span>'
    )


# ── HEADER: Accuracy Gauge + Key Metrics ─────────────────────────────────────
st.markdown('<div style="font-family:IBM Plex Mono,monospace;color:#3b82f6;font-size:0.75rem;letter-spacing:2px;margin-bottom:0.5rem;">FACT-CHECK REPORT</div>', unsafe_allow_html=True)
...
with header_col1:
    ...
    fig_gauge = go.Figure(go.Indicator(
        mode="gauge+number",
        value=overall,
        title={"text": "Credibility Score", "font": {"family": "IBM Plex Mono", "color": "#94a3b8", "size": 13}},
        number={"font": {"family": "IBM Plex Mono", "size": 48, "color": score_color}, "suffix": ""},
        gauge={
...
with header_col2:
    ...
    st.markdown(f"""
    <div style="background:rgba(0,0,0,0.3);border:1px solid #1e2d4a;border-radius:8px;padding:0.75rem 1rem;margin-top:0.5rem;">
    <div style="font-family:IBM Plex Mono,monospace;font-size:0.7rem;color:#475569;margin-bottom:0.25rem;">AI TEXT PROBABILITY</div>
    <div style="display:flex;align-items:center;gap:1rem;">
      <div style="font-family:IBM Plex Mono,monospace;font-size:1.5rem;font-weight:700;color:{ai_color};">{ai_prob:.0f}%</div>
      <div style="font-size:0.82rem;color:#94a3b8;">{ai_label}</div>
    </div>
    </div>
    """, unsafe_allow_html=True)
...
if narrative:
    st.markdown(f"""
    <div class="fc-card">
    <div style="font-family:IBM Plex Mono,monospace;color:#475569;font-size:0.72rem;letter-spacing:2px;margin-bottom:0.75rem;">EXECUTIVE SUMMARY</div>
    <div style="color:#cbd5e1;font-size:1rem;line-height:1.8;font-style:italic;border-left:3px solid #3b82f6;padding-left:1rem;">{narrative}</div>
    </div>
    """, unsafe_allow_html=True)
...
with chart_col1:
    ...
    if labels:
        ...
        fig_pie = go.Figure(data=[go.Pie(
            ...
            textfont={'color':"#e2e8f0", 'family':"IBM Plex Mono"},
...
with chart_col2:
    ...
    fig_bar.update_traces(textposition='outside', textfont={'color': '#e2e8f0'})
    fig_bar.update_layout(
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        margin=dict(t=20, b=20, l=20, r=20),
        height=300,
        xaxis=dict(title='', showgrid=False, tickfont={'color': '#94a3b8'}),
        yaxis=dict(title='Claims', showgrid=True, gridcolor='#1e2d4a', tickfont={'color': '#94a3b8'}),
        showlegend=False
    )
...
if all_evidence:
    ...
    for e in all_evidence:
        ...
        label = {1: "Tier 1 (Authoritative)", 2: "Tier 2 (Reputable)", 
                 3: "Tier 3 (Moderate)", 4: "Tier 4 (Unknown)"}.get(tier, "Unknown")
    ...
    tier_colors_map = {
        "Tier 1 (Authoritative)": "#10b981",
        "Tier 2 (Reputable)": "#3b82f6",
        "Tier 3 (Moderate)": "#f59e0b",
        "Tier 4 (Unknown)": "#475569"
    }
    ...
    fig_tier.update_layout(
        ...
        margin=dict(t=20, b=20, l=20, r=20),
        height=250,
        xaxis=dict(title='', tickfont={'color': '#94a3b8', 'size': 12}),
        yaxis=dict(title='Count', showgrid=True, gridcolor='#1e2d4a', tickfont={'color': '#94a3b8'}),
    )
...
for claim in filtered:
    ...
    with st.expander(
        ...
        with card_col1:
            ...
            if audited:
                st.warning("⚠️ This verdict was flagged by the hallucination self-audit and may have been corrected.")
        ...
        with card_col2:
            ...
            st.markdown(f"""
            <div style="font-family:IBM Plex Mono,monospace;font-size:0.7rem;color:#64748b;margin-bottom:4px;">CONFIDENCE</div>
            <div style="font-family:IBM Plex Mono,monospace;font-size:1.8rem;font-weight:700;color:{bar_color};">{conf}%</div>
            <div style="background:#1e2d4a;border-radius:4px;height:6px;margin-top:6px;">
        ...
        if evidence:
            ...
            for ev in evidence[:4]:
                ...
                tier_color = {1: "#10b981", 2: "#3b82f6", 3: "#f59e0b", 4: "#64748b"}.get(tier, "#64748b")
                method_color = {"bs4": "#3b82f6", "selenium": "#f97316", "playwright": "#a855f7", "scrapling": "#10b981"}.get(ev.get("method",""), "#64748b")

                is_supporting = ev.get("url") in citations
                is_contradicting = ev.get("url") in contradicting
                border = "#10b981" if is_supporting else "#ef4444" if is_contradicting else "#1e2d4a"

                st.markdown(f"""
                <div style="border:1px solid {border};border-radius:6px;padding:0.5rem 0.75rem;margin-bottom:0.4rem;background:#050810;">
                  <div style="display:flex;gap:0.5rem;align-items:center;flex-wrap:wrap;">
                    <span style="color:{method_color};font-family:IBM Plex Mono,monospace;font-size:0.68rem;background:rgba(0,0,0,0.4);padding:1px 6px;border-radius:4px;">{ev.get('method','').upper()}</span>
                    <span style="color:{tier_color};font-family:IBM Plex Mono,monospace;font-size:0.68rem;">T{tier}</span>
                    <a href="{ev.get('url','#')}" target="_blank" style="color:#60a5fa;font-size:0.8rem;text-decoration:none;flex:1;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">{ev.get('domain', ev.get('url',''))}</a>
...
if st.button("🔄 NEW CHECK — Run another fact-check"):
